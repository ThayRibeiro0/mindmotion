...
const Dashboard: React.FC = (): JSX.Element => {
  ...
  useEffect(() => {
    axios.get<{ q: string; a: string }>('/api/quotes').then((res: { data: { q: any; a: any; }; }) => {
      setQuote(`${res.data.q} — ${res.data.a}`);
    });
    axios.get<{ total: number; todayCount: number; streak: number; logs: any[] }>('/api/meditations/stats').then((res: { data: React.SetStateAction<{ total: number; todayCount: number; streak: number; logs: any[]; }>; }) => {
      setStats(res.data);
    });
  }, []);
